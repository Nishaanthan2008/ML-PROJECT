...
def seed_database(reset_db=False):
    ...
    with app.app_context():
        ...
        if reset_db:
            print("--> WARNING: Reset requested. Dropping all tables...")
            db.drop_all()
        db.create_all()

        admin = User.query.filter_by(email='admin@profileshield.ai').first()
        if admin:
        ...
        dataset_record = Dataset.query.first()
        df = None
        if not dataset_record:
            ...
            try:
                ...
                print(f"    Notice: Could not write dataset file to disk: {e}")
            ...
            print("--> Dataset metadata record already exists.")

        model_file = os.path.join(app.config['MODEL_FOLDER'], 'best_profile_shield_model.joblib')
        model_record = MLModelRegistry.query.first()

        if not model_record or not os.path.exists(model_file):
            ...
            print("--> ML Model Registry record and model file already exist.")

        if not ProfileAnalysis.query.first():
            ...
            print("--> Sample profile assessments already exist.")
