...
def _load_model_and_pipeline():
    """Load the saved ML model and feature pipeline."""
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for idx, row in df.iterrows():
                ...
                posts_count  = max(int(row.get('posts_count', 1)), 1)  # never 0
                ...
                if model is not None:
                    ...
                    following = profile_input['following_count']
                    followers = profile_input['followers_count']
                    bot_prob  = 0.85 if following > followers * 3 else 0.25
                ...
                analysis = ProfileAnalysis(
                    ...
                    user_id=current_user.id,   # ← data isolation
            ...
            db.session.commit()
            flash(
                f'Batch processing complete! Analysed {processed_count} profiles.',
                'success',
            )
            ...
            db.session.rollback()
            flash(f'Error processing CSV: {str(exc)}', 'danger')
            return redirect(url_for('batch.upload_csv'))
...
@login_required
def history():
    """Searchable, filterable, paginated history of the current user's profile scans.
    ...
    if current_user.is_admin():
        ...
        query = ProfileAnalysis.query.filter_by(user_id=current_user.id)
